...
class GoLogin(object):
    def __init__(self, options: Dict[str, Union[str, bool, int]]):
        ...
        self.access_token: str = str(options.get("token"))
        self.profile_id: str = str(options.get("profile_id"))
        self.tmpdir: str = str(options.get("tmpdir", tempfile.gettempdir()))  # type: ignore
        self.address: str = str(options.get("address", "127.0.0.1"))
        self.extra_params: List[Any] = options.get("extra_params", [])  # type: ignore
        self.port: int = int(options.get("port", 3500))
        self.local: bool = bool(options.get("local", False))
        self.spawn_browser: bool = bool(options.get("spawn_browser", True))
        self.credentials_enable_service = options.get(
            "credentials_enable_service"
        )
        self.cleaningLocalCookies: bool = bool(
            options.get("cleaningLocalCookies", False)
        )
        self.uploadCookiesToServer: bool = bool(
            options.get("uploadCookiesToServer", False)
        )
        self.writeCookiesFromServer: bool = bool(
            options.get("writeCookiesFromServer", False)
        )
        self.restore_last_session: bool = bool(
            options.get("restore_last_session", False)
        )
        self.executablePath: str = ""
        self.is_cloud_headless: bool = bool(
            options.get("is_cloud_headless", True)
        )
        self.is_new_cloud_browser: bool = bool(
            options.get("is_new_cloud_browser", True)
        )

        home = str(pathlib.Path.home())
        browser_gologin = os.path.join(home, ".gologin_test", "browser")
        try:
        ...
        self.setProfileId(self.profile_id)
        self.preferences = {}
        self.pid = int()
    ...
    def setProfileId(self, profile_id: str) -> None:
    ...
    def loadExtensions(self):
        ...
        pathToExt = ""
        profileExtensionsCheck: List[Any] = []
        for ext in chromeExtensions:
            try:
                ver = extensionsManagerInst.downloadExt(ext)  # type: ignore
        ...
        noteExtExist = ExtensionsManager().extensionIsAlreadyExisted(  # type: ignore
    ...
    def spawnBrowser(self):
        ...
        if proxy:
            ...
            params.append("--proxy-server=" + str(proxy))
        ...
        while try_count < 100:
            try:
                data = requests.get("http://" + url + "/json").content  # type: ignore
                break
            except:
                try_count += 1
                time.sleep(1)
    ...
    def waitUntilProfileUsing(self, try_count: int = 0):
        if try_count > 10:
            return
        time.sleep(1)
    ...
    def formatProxyUrlPassword(self, proxy: Dict[str, Any]) -> str:
        mode = (
            "socks5h"
            if proxy.get("mode") == "socks5"
            else proxy.get("mode", "http")
        )
    ...
    def getProfile(self, profile_id: Union[str, None] = None):
    ...
    def downloadProfileZip(self):
        ...
        data = requests.get(
            FILES_GATEWAY + "/download", headers=headers
        ).content
    ...
    def convertPreferences(
        ...
        preferences["webgl_noise_value"] = preferences.get("webGL", {}).get(
            "noise"
        )
        preferences["get_client_rects_noise"] = preferences.get(
            "webGL", {}
        ).get("getClientRectsNoise")
        ...
        preferences["webgl"] = {
            "metadata": {
                ...
                "renderer": preferences.get("webGLMetadata", {}).get(
                    "renderer"
                ),
                "mode": preferences.get("webGLMetadata", {}).get("mode")
                == "mask",
            }
        }

        if preferences.get("navigator", {}).get("userAgent"):
            preferences["userAgent"] = preferences.get("navigator", {}).get(
                "userAgent"
            )
        ...
        if preferences.get("navigator", {}).get("language"):
            preferences["languages"] = preferences.get("navigator", {}).get(
                "language"
            )
        ...
        if preferences.get("os") == "android":
            devicePixelRatio: float = preferences.get("devicePixelRatio")  # type: ignore
    ...
    def updatePreferences(self):
        ...
        if "navigator" in profile:
            if "deviceMemory" in profile["navigator"]:
                profile["deviceMemory"] = (
                    profile["navigator"]["deviceMemory"] * 1024
                )
        ...
        if "gologin" in preferences:
            if "navigator" in preferences["gologin"]:
                if "deviceMemory" in preferences["gologin"]["navigator"]:
                    profile["deviceMemory"] = (
                        preferences["gologin"]["navigator"]["deviceMemory"]
                        * 1024
                    )
            if "deviceMemory" in preferences["gologin"]:
                profile["deviceMemory"] = preferences["gologin"][
                    "deviceMemory"
                ]

        proxy: Union[Dict[str, Any], None] = self.profile.get("proxy")
        # print('proxy=', proxy)
        if proxy and (
            proxy.get("mode") == "gologin" or proxy.get("mode") == "tor"
        ):
        ...
        if self.credentials_enable_service != None:
            preferences["credentials_enable_service"] = (
                self.credentials_enable_service
            )
    ...
    def downloadCookies(self):
        ...
        try:
            ...
            cookiesManagerInst.write_cookies_to_file(cookies)  # type: ignore
        except Exception as e:
            print("downloadCookies exc", e, e.__traceback__.tb_lineno)  # type: ignore
            raise e

    def uploadCookies(self, cookies: List[Dict[str, Any]]):
        api_base_url = API_URL
    ...
    def getRandomFingerprint(self, options: Dict[str, Any]):
        os_type = options.get("os", "lin")
        return json.loads(
            requests.get(
                API_URL + "/browser/fingerprint?os=" + os_type,
    ...
    def create(self, options: Dict[str, Any] = {}):
        ...
        navigator: Dict[str, Any] = options.get("navigator")  # type: ignore
    ...
    def delete(self, profile_id: Union[str, None] = None):
        profile = self.profile_id if profile_id == None else profile_id
        requests.delete(
    ...
    def update(self, options: Dict[str, Any]):
        self.profile_id = options.get("id")  # type: ignore
        profile = self.getProfile()
        # print("profile", profile)
        for k, v in options.items():
            profile[k] = v
        resp = requests.put(  # type: ignore
    ...
    def startRemote(self, delay_s: int = 3):
        ...
        remote_orbita_url = (
            "https://" + self.profile_id + ".orbita.gologin.com"
        )
        ...
        return self.waitDebuggingUrl(
            delay_s, remote_orbita_url=remote_orbita_url
        )

    def stopRemote(self):
        response = requests.delete(  # type: ignore
            API_URL + "/browser/" + self.profile_id + "/web",
            headers=self.headers(),
            params={"isNewCloudBrowser": self.is_new_cloud_browser},
        )

    def clearCookies(self, profile_id: Optional[str] = None):
    ...
    async def normalizePageView(self, page: Any):
